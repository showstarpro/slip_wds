...
def get_args_parser():
    ...
    parser.add_argument( "--device", default="cuda", type=str, help="Accelerator to use.")
    parser.add_argument('--ms-coco', action='store_true', help='eval mscoco')
    parser.add_argument('--flickr', action='store_true', help='eval flickr')
...
def main(args):
    ...
    val_dataset = datasets.get_downstream_dataset(catalog, name=d, is_train=False, transform=val_transform, tokenizer=tokenizer)
    if args.ms_coco or args.flickr:
        k_vals=[1, 5, 10]
        t2i, i2t = recall_at_k(model, val_dataset, args.device, k_vals, args.batch_size)
    ...
    templates = all_templates['openai']
    labels = all_labels[d]

    # is_acc = d not in ['aircraft', 'pets', 'caltech101', 'flowers', 'kinetics700_frames', 'hateful_memes']

    acc_or_outputs = validate_zeroshot(val_loader, templates, labels, model, tokenizer, is_acc=False)
